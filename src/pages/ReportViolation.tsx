...
import { addReport } from "@/lib/activityStore";
...
const ReportViolation = () => {
  ...
  const handleSubmit = () => {
    const report = addReport({
      ...
      description,
      category,
      hasEvidence: !!evidencePreview,
    });
    setRefId(report.refId);
    setSubmitted(true);
    toast({ title: "Report submitted!", description: `Reference: ${report.refId}` });
  };
        ...
        <section className="hero-bg min-h-[calc(100vh-4rem)] flex items-center justify-center py-16">
          ...
          >
            ...
            <p className="text-muted-foreground mb-6">
              Your environmental violation report has been filed successfully.
              We'll keep you updated on its progress.
            </p>
            <p className="text-sm text-muted-foreground mb-6">
              Reference ID: <span className="font-mono font-semibold text-primary">{refId}</span>
            </p>
            <Button
              onClick={() => {
                setSubmitted(false);
        ...
        <div className="container mx-auto px-4 max-w-2xl">
          ...
          >
            <h1 className="text-3xl md:text-5xl font-extrabold mb-4">
              Report <span className="eco-gradient-text">Violation</span>
            </h1>
            <p className="text-muted-foreground">
              Help protect the environment by reporting violations in your area.
            </p>
          </motion.div>
          ...
          <div className="flex items-center justify-between mb-10 px-4">
            {steps.map((s, i) => (
              <div key={s.label} className="flex items-center gap-2">
                ...
                {i < steps.length - 1 && (
                  <div
                    className={`hidden sm:block w-8 md:w-16 h-0.5 mx-1 rounded-full ${
          ...
          <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="glass-card p-8">
            <AnimatePresence mode="wait">
              ...
              {step === 1 && (
                <div className="space-y-4">
                  ...
                  <Textarea
                    placeholder="Provide a detailed description of the environmental violation..."
                    value={description}
              ...
              {step === 2 && (
                <div className="space-y-4">
                  ...
                  {!evidencePreview ? (
                    <div
                      className="border-2 border-dashed border-primary/30 rounded-2xl p-12 text-center cursor-pointer hover:border-primary/60 transition-colors"
            ...
            <div className="flex justify-between mt-8">
              ...
              {step < 3 ? (
                ...
                <Button onClick={handleSubmit} disabled={!canNext()} className="rounded-xl">
